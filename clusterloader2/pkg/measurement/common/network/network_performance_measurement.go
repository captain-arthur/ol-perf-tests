...
const (
	...
	initialDelayForTestExecution = 30 * time.Second
...
func (npm *networkPerformanceMeasurement) start(config *measurement.Config) error {
	...
	klog.Infof("Preparing cluster for network measurement...")
	if err := npm.prepareCluster(); err != nil {
		return err
	}
	klog.Infof("Waiting 10s for RBAC and CRD propagation...")
	time.Sleep(10 * time.Second)
	klog.Infof("Creating worker pods...")
...
func (npm *networkPerformanceMeasurement) gather() (measurement.Summary, error) {
	if err := npm.waitForMetricsFromPods(); err != nil {
		return nil, err
	}
...
func (npm *networkPerformanceMeasurement) createCustomResourcePerUniquePodPair(uniquePodPairList []podPair) {
	...
	templateData, err := fs.ReadFile(manifestsFS, networkTestRequestFilePath)
	if err != nil {
		klog.Errorf("Failed to read template file %s: %v", networkTestRequestFilePath, err)
		return
	}

	// Create a text/template that allows dynamic keys (bypassing safetext/yamltemplate)
	tmpl, err := template.New("network-test-request").Parse(string(templateData))
	if err != nil {
	...
	for pairIndex, podPair := range uniquePodPairList {
		...
		var b bytes.Buffer
		if err := tmpl.Execute(&b, templateMapping); err != nil {
			klog.Errorf("Failed to execute template for pair %d: %v", pairIndex, err)
			continue
		}

		obj := &unstructured.Unstructured{}
		if err := yaml.Unmarshal(b.Bytes(), obj); err != nil {
			klog.Errorf("Failed to unmarshal templated YAML: %v", err)
			continue
		}

		if err := npm.framework.CreateObject(obj.GetNamespace(), obj.GetName(), obj); err != nil {
			klog.Errorf("Failed to create NetworkTestRequest: %v", err)
		}
	}
}
...
func (npm *networkPerformanceMeasurement) waitForMetricsFromPods() error {
	...
	if err := wait.Poll(interval, timeout, npm.checkResponseReceivedFromPods); err != nil {
		return cl2errors.NewErrCritical(fmt.Errorf("failed to receive response from %v pods", npm.numberOfClients-len(npm.metricVal)))
	}
	return nil
}
