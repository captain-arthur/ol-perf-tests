...
func (wp *workerPodInfo) formUniquePodPair() []podPair {
	...
	// Single node case: pair pods from the same node
	if nh.Len() == 1 {
		node := nh[0]
		for i := 0; node.numberOfPods > 1; i++ {
			sourcePod, err1 := wp.pop(node.nodeName)
			destinationPod, err2 := wp.pop(node.nodeName)
			if err1 != nil || err2 != nil {
				klog.Errorf("error in popping pods from same node. err1: %v, err2: %v", err1, err2)
				break
			}
